...
# drop columns
home.processed2[,c("Loan_ID","Gender","CoapplicantIncome", "ApplicantIncome", "Loan_Amount_Term", "Loan_Status", "LoanMonthly"):=NULL]
...
## search for optimum
```{r}
plotcp(tree.cat.m1) # we can see an optimal xerror @ approx CP=0.0030303 with xerror=0.081818
```

## prune tree
```{r}
# choose cp ~ 0.0031 since it is the simplest tree given the CV error cap (xerror + xstd)
cp1 <- 0.0031
...
## confusion matrix
```{r}
tree.cat.m1.pred <- data.table(predict(tree.cat.m1, home.cat.cart.test))
