...
home.processed3[,c("Loan_ID", "Loan_Amount_Term", "LoanAmount"):=NULL]
```
...
home.cont.cart <- createDataPartition(home.processed3$LoanMonthly, p=0.8, list=FALSE)
home.cont.cart.train <- home.processed3[home.cont.cart,]
home.cont.cart.test <- home.processed3[-home.cont.cart,]

tree.cont.m1 <- rpart(LoanMonthly ~ ., data = home.cont.cart.train, method = "anova", control = rpart.control(minsplit = 10, cp = 0))
...
```
```{r}
tree.cont.m1.pruned.pred <- predict(tree.cont.m1, home.cont.cart.test)

data.frame(R2=R2(tree.cont.m1.pruned.pred, home.cont.cart.test$LoanMonthly), RMSE=RMSE(tree.cont.m1.pruned.pred, home.cont.cart.test$LoanMonthly), MAE=MAE(tree.cont.m1.pruned.pred, home.cont.cart.test$LoanMonthly))

mean(home.cont.cart.test$LoanMonthly)
```
