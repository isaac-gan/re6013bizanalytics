...
eligibility.train.dt[, loanmonthly.bracket := cut(LoanMonthly, breaks = loanmonthly.cutoffs, include.lowest = TRUE)]

# drop columns
eligibility.train.dt[,c("Gender","CoapplicantIncome", "ApplicantIncome", "Loan_Amount_Term", "LoanMonthly"):=NULL]
summary(eligibility.train.dt)
```


##Implement Log Reg to compare against CART performance
```{r}
eligibility.m3 <- glm(Loan_Status ~ ., family = binomial, data = eligibility.train.dt)
summary(eligibility.m3)
```
```{r}
OR.m3 <- exp(coef(eligibility.m3))
OR.m3

OR.CI.m3 <- exp(confint(eligibility.m3))
OR.CI.m3

prob <- predict(eligibility.m3, type = 'response')

plot(x=eligibility.test.dt$Loan_Status,y=prob,type="1",main='Log Reg Prob of Being Eligible for Loan')
